...
1. Read the nearest project `AGENTS.md`, matching Engineering Rules, and the
   Runtime contract.
...
7. Before a mutate action, show the exact frames or UDS request set and obtain
   explicit external-state-change confirmation. Pass both `--require-confirm`
   and `--confirm` only after that gate is satisfied.
...
- Accept native driver paths only from the Runtime-owned `can-drivers.json`;
  never source a DLL from the project, a task argument, or task environment.
...
- Keep CAN transmit and UDS activity behind explicit external-state-change
  confirmation.
