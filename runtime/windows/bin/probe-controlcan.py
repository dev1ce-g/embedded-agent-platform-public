...
def probe(path: Path, device_types: list[int], indexes: list[int], try_init: bool, can_index: int) -> list[dict[str, Any]]:
    ...
    with tempfile.TemporaryDirectory(prefix="controlcan-probe-") as temporary:
        copied = Path(temporary) / "ControlCAN.dll"
        shutil.copy2(path, copied)
        directories = [copied.parent, path.parent, path.parent / "kerneldlls", path.parent / "plugin", path.parent.parent, path.parent.parent / "kerneldlls", path.parent.parent / "plugin"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    paths = [Path(value) for value in (args.Dll or DEFAULT_DLLS) if value]
    if args.mode in {"one", "range"} and not args.Dll:
        parser.error(f"{args.mode} requires -Dll")
    ...
    value = {"ok": not any("error" in item for item in results), "operation": "probe-controlcan", "exit_code": 0 if results else 2, "mode": args.mode, "process_arch": "x86" if struct.calcsize("P") == 4 else "x64", "inventory": inventory, "results": results}
    if args.mode == "range":
